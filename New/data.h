...
struct Car {
    ...
    // CSV format: id|brand|model|year|price|seller|status
    ...
    string to_line() const {
        // id|brand|model|year|price|seller|status
        ostringstream os;
    ...
    // dynamic price based on age: after 3 years, 5% reduction per year
...
struct Booking {
    ...
    // bookingId|carId|buyer|date|status
...
// Helpers: read all cars from cars.txt
...
// deliveries: treat as stack, but stored in file
...
inline void log_action(const string &s) {
    ...
    time_t t = time(nullptr);
    out << "[" << ctime(&t);
    out.seekp(-1, ios::cur); // remove newline from ctime
    out << "] " << s << '\n';
}

// helper for generating new unique IDs (read max from file)
inline int next_car_id() {
    auto cars = read_all_cars();
    int mx = 100; // start id at 101 by default in examples
